...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-04T20:13:51+00:00",
...
## [שאלון לפני ההרצאה](https://purple-hill-04aebfb03.1.azurestaticapps.net/quiz/6)
...
תיאור התפלגות ההסתברות של משתנה רציף, עם ערכים הנלקחים מטווח [a,b] או מכל קבוצת המספרים הממשיים ℝ, הוא מורכב יותר. לדוגמה, במקרה של זמן הגעת האוטובוס, ההסתברות שהאוטובוס יגיע בדיוק בזמן מסוים *t* היא למעשה 0!

> עכשיו אתם יודעים שאירועים עם הסתברות 0 קורים, ולעיתים קרובות! לפחות בכל פעם שהאוטובוס מגיע!

ניתן לדבר רק על ההסתברות שמשתנה ייפול בטווח מסוים של ערכים, לדוגמה P(t<sub>1</sub>≤X<t<sub>2</sub>). במקרה זה, התפלגות ההסתברות מתוארת באמצעות **פונקציית צפיפות הסתברות** p(x), כך ש-
...
נניח שאנו לוקחים רצף של n דגימות של משתנה מקרי X: x<sub>1</sub>, x<sub>2</sub>, ..., x<sub>n</sub>. ניתן להגדיר את **הממוצע** (או **הממוצע האריתמטי**) של הרצף בדרך המסורתית כ-(x<sub>1</sub>+x<sub>2</sub>+...+x<sub>n</sub>)/n. ככל שנגדיל את גודל המדגם (כלומר ניקח את הגבול כאשר n→∞), נקבל את הממוצע (שנקרא גם **תוחלת**) של ההתפלגות. נסמן את התוחלת כ-**E**(x).
...
כאן אנו מחשבים גם את **טווח הרבעונים** IQR=Q3-Q1, ואת מה שנקרא **ערכים חריגים** - ערכים שנמצאים מחוץ לטווח [Q1-1.5*IQR,Q3+1.5*IQR].
...
> **הערה**: כדי לראות דוגמה לעבודה עם מערך נתונים זה, עיינו ב-[מחברת המצורפת](notebook.ipynb). לאורך השיעור ישנם אתגרים שתוכלו להשלים על ידי הוספת קוד למחברת זו. אם אינכם בטוחים כיצד לעבוד עם נתונים, אל דאגה - נחזור לעבודה עם נתונים באמצעות Python בהמשך. אם אינכם יודעים כיצד להריץ קוד ב-Jupyter Notebook, עיינו ב-[מאמר זה](https://soshnikov.com/education/how-to-execute-notebooks-from-github/).

... 

(המשך התרגום בהתאם למבנה הטקסט המקורי)
> **רווח סמך** הוא הערכה של הממוצע האמיתי של האוכלוסייה בהתבסס על המדגם שלנו, שהיא מדויקת ברמת הסתברות מסוימת (או **רמת ביטחון**).
בהנחה שיש לנו מדגם X<sub>1</sub>, ..., X<sub>n</sub> מההתפלגות שלנו. בכל פעם שניקח מדגם מההתפלגות, נקבל ערך ממוצע שונה μ. לכן ניתן להתייחס ל-μ כמשתנה מקרי. **רווח סמך** עם רמת ביטחון p הוא זוג ערכים (L<sub>p</sub>,R<sub>p</sub>), כך ש-**P**(L<sub>p</sub>≤μ≤R<sub>p</sub>) = p, כלומר ההסתברות שהממוצע שנמדד ייפול בתוך הרווח שווה ל-p.

הדיון על איך מחשבים את רווחי הסמך חורג מהמבוא הקצר שלנו. ניתן למצוא פרטים נוספים [בויקיפדיה](https://en.wikipedia.org/wiki/Confidence_interval). בקצרה, אנו מגדירים את ההתפלגות של ממוצע המדגם המחושב ביחס לממוצע האמיתי של האוכלוסייה, שנקראת **התפלגות סטודנט**.
...
> **הערה**: אנו גם מדלגים על דיון במושג חשוב של [דרגות חופש](https://en.wikipedia.org/wiki/Degrees_of_freedom_(statistics)), שהוא משמעותי בהקשר להתפלגות סטודנט. ניתן לעיין בספרים מקיפים יותר על סטטיסטיקה כדי להבין את המושג לעומק.
...
במאגר הנתונים של שחקני הבייסבול שלנו, ישנם תפקידים שונים שניתן לסכם בטבלה הבאה (ראו את [המחברת המצורפת](notebook.ipynb) כדי לראות איך הטבלה חושבה):
...
אחת הסיבות לכך שהתפלגות נורמלית חשובה היא **משפט הגבול המרכזי**. בהנחה שיש לנו מדגם גדול של N ערכים X<sub>1</sub>, ..., X<sub>N</sub>, שנדגמו מכל התפלגות עם ממוצע μ ושונות σ<sup>2</sup>. אז, עבור N גדול מספיק (במילים אחרות, כאשר N→∞), הממוצע Σ<sub>i</sub>X<sub>i</sub> יתפלג נורמלית, עם ממוצע μ ושונות σ<sup>2</sup>/N.
