...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-24T13:56:34+00:00",
...
即使数据科学家谨慎选择了适合数据的正确图表类型，数据仍然可能被以某种方式展示以证明某个观点，通常会以牺牲数据本身为代价。有许多误导性图表和信息图的例子！
...
这个臭名昭著的例子同时使用了颜色和颠倒的 Y 轴来误导：本应得出枪击死亡人数在通过支持枪支的立法后激增的结论，实际上视觉上却让人误以为情况正好相反：
...
你在上面“佛罗里达枪支暴力”图表中看到，颜色可以为图表提供额外的意义层次，尤其是那些没有使用 ggplot2 和 RColorBrewer 等库设计的图表，这些库自带各种经过验证的颜色库和调色板。如果你手动制作图表，可以稍微研究一下[颜色理论](https://colormatters.com/color-and-design/basic-color-theory)。

> ✅ 在设计图表时，要注意可访问性是可视化的重要方面。你的部分用户可能是色盲——你的图表是否对视觉障碍用户友好？

选择图表颜色时要小心，因为颜色可能传递你未曾预料的含义。上面“身高”图表中的“粉色女士”传递了一种明显的“女性化”意义，进一步增加了图表本身的怪异感。

虽然[颜色的意义](https://colormatters.com/color-symbolism/the-meanings-of-colors)可能因地区而异，并且根据色调的不同而变化，但一般来说，颜色的意义包括：
...
如果你需要为图表选择自定义颜色，请确保你的图表既可访问，又与试图传递的意义相符。
...
如果你的数据在 X 轴上是文本且冗长，可以将文本倾斜以提高可读性。[plot3D](https://cran.r-project.org/web/packages/plot3D/index.html) 提供了 3D 绘图功能，如果你的数据支持它，可以使用它来生成复杂的数据可视化。

![3d plots](../../../../../3-Data-Visualization/R/13-meaningful-vizualizations/images/3d.png)

## 动画和3D图表显示

如今一些最佳的数据可视化是动画化的。Shirley Wu 使用 D3 制作了令人惊叹的可视化，例如“[电影之花](http://bl.ocks.org/sxywu/raw/d612c6c653fb8b4d7ff3d422be164a5d/)”，每朵花都是一部电影的可视化。另一个为《卫报》制作的例子是“Bussed Out”，一个结合了 Greensock 和 D3 的交互式体验，以及滚动叙事文章格式，展示了纽约市如何通过将无家可归者送出城市来处理无家可归问题。
...
虽然本课不足以深入教授这些强大的可视化库，但你可以尝试在 Vue.js 应用中使用 D3，展示一本书《危险关系》的动画社交网络可视化。

> 《危险关系》是一部书信体小说，即以书信形式呈现的小说。由 Choderlos de Laclos 于1782年创作，讲述了18世纪末法国贵族中两位主角——瓦尔蒙子爵和梅尔特伊侯爵夫人——之间恶毒、道德败坏的社会操纵故事。两人最终都走向毁灭，但在此之前造成了巨大的社会破坏。小说通过他们写给圈内各种人的信件展开，信件内容包括复仇计划或单纯制造麻烦。创建这些信件的可视化，发现叙事中的主要关键人物。

你将完成一个网络应用，显示这个社交网络的动画视图。它使用一个库来创建[网络的可视化](https://github.com/emiliorizzo/vue-d3-network)，基于 Vue.js 和 D3。当应用运行时，你可以在屏幕上拖动节点以重新排列数据。
...
浏览互联网，发现误导性可视化。作者如何欺骗用户，这是否是故意的？尝试修正这些可视化，展示它们应该如何呈现。
