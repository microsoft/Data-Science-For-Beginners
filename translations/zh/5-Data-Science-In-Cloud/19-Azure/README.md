...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-24T13:09:23+00:00",
...
|:---:|
| 云端数据科学：Azure ML SDK - _由 [@nitya](https://twitter.com/nitya) 绘制的速写笔记_ |
...
- 使用自动化机器学习，提供配置参数和训练数据。它会自动迭代算法和超参数设置，找到最佳模型进行预测。
...
在[上一课](../18-Low-Code/README.md)中，我们学习了如何以低代码/无代码方式训练、部署和消费模型。我们使用了心力衰竭数据集生成了心力衰竭预测模型。在本课中，我们将使用 Azure Machine Learning SDK 完成相同的任务。
...
恭喜你，你刚刚创建了一个计算实例！我们将在[创建笔记本部分](../../../../5-Data-Science-In-Cloud/19-Azure)中使用这个计算实例。
...
要创建笔记本，我们需要一个计算节点来提供 Jupyter Notebook 实例。返回 [Azure ML 工作区](https://ml.azure.com/)，点击计算实例。在计算实例列表中，你应该能看到我们之前创建的[计算实例](../../../../5-Data-Science-In-Cloud/19-Azure)。

1. 在 Applications 部分，点击 Jupyter 选项。
2. 勾选 "Yes, I understand" 复选框并点击 Continue 按钮。
...
要从工作区获取或创建实验，你需要使用实验名称请求实验。实验名称必须是 3-36 个字符，以字母或数字开头，并且只能包含字母、数字、下划线和短横线。如果在工作区中找不到实验，会创建一个新的实验。
...
如文档所述，有许多参数可以调整。对于本项目，我们将使用以下参数：
...
通过 SDK 你可以做很多其他事情，但遗憾的是我们无法在本课中全部讲解。不过好消息是，学会快速浏览 SDK 文档可以让你在学习中走得更远。查看 Azure ML SDK 文档，找到允许你创建管道的 `Pipeline` 类。管道是可以作为工作流执行的一系列步骤的集合。
...
在本课中，你学习了如何使用 Azure ML SDK 在云端训练、部署和消耗一个模型来预测心脏病风险。查看这个 [文档](https://docs.microsoft.com/python/api/overview/azure/ml/?view=azure-ml-py?WT.mc_id=academic-77958-bethanycheum&ocid=AID3041109)，获取关于 Azure ML SDK 的更多信息。尝试使用 Azure ML SDK 创建你自己的模型。
