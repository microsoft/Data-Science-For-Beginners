...
> **關於測驗的說明**：所有測驗都包含在 Quiz-App 文件夾中，共有 40 個測驗，每個測驗包含三個問題。測驗在課程中有鏈接，但測驗應用可以在本地運行或部署到 Azure；請按照 `quiz-app` 文件夾中的指導進行操作。測驗正在逐步本地化。
...
1. 如果您是第一次使用開發容器，請確保您的系統符合前置要求（例如已安裝 Docker），詳情請參閱 [入門文檔](https://code.visualstudio.com/docs/devcontainers/containers#_getting-started)。
