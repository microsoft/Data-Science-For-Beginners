...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-24T13:55:30+00:00",
...
이 강의에서는 다양한 R 패키지 라이브러리를 사용하여 수량 개념을 중심으로 흥미로운 시각화를 만드는 방법을 탐구합니다. 미네소타의 새들에 대한 정리된 데이터셋을 사용하여 지역 야생동물에 대한 많은 흥미로운 사실을 배울 수 있습니다.  
...
✅ 이 [cheatsheet](https://nyu-cdsc.github.io/learningr/assets/data-visualization-2.1.pdf)를 참고하여 ggplot2에 대한 설명을 확인하세요.

## 새 날개 길이 값을 기반으로 선형 플롯 만들기
...
이 흥미로운 새들의 최대 날개 길이를 시각화하는 기본 선형 플롯을 만들어 봅시다.
...
ggplot(data=birds, aes(x=Name, y=MaxWingspan,group=1)) +
  geom_line() 
```  
여기서 `ggplot2` 패키지를 설치한 후 `library("ggplot2")` 명령을 사용하여 작업 공간에 가져옵니다. ggplot에서 플롯을 생성하려면 `ggplot()` 함수를 사용하며 데이터셋, x 및 y 변수 등을 속성으로 지정합니다. 이 경우, 선형 플롯을 생성하려고 하므로 `geom_line()` 함수를 사용합니다.

![MaxWingspan-lineplot](../../../../../3-Data-Visualization/R/09-visualization-quantities/images/MaxWingspan-lineplot.png)

즉시 무엇을 발견할 수 있나요? 적어도 하나의 이상치가 있는 것 같습니다. 날개 길이가 상당히 깁니다! 2000+ 센티미터의 날개 길이는 20미터 이상에 해당합니다. 미네소타에 프테로닥틸이 돌아다니고 있는 걸까요? 조사해 봅시다.
...
레이블을 45도 회전으로 설정했음에도 불구하고 너무 많아 읽기 어렵습니다. 다른 전략을 시도해 봅시다: 이상치만 레이블을 지정하고 차트 내에 레이블을 설정합니다. 산점도를 사용하여 레이블링 공간을 더 확보할 수 있습니다:
...
ggplot(data=birds, aes(x=Name, y=MaxWingspan,group=1)) +
  ...
  ggtitle("Max Wingspan in Centimeters") + 
```  
여기서 `geom_point()` 함수를 사용하여 산점도를 플롯합니다. 이를 통해 `MaxWingspan > 500`인 새들에 대한 레이블을 추가하고 x축의 레이블을 숨겨 플롯을 깔끔하게 정리했습니다.
...
Bald Eagle과 Prairie Falcon은 아마도 매우 큰 새일 가능성이 높지만, 최대 날개 길이에 0이 추가된 잘못된 레이블로 보입니다. 25미터 날개 길이를 가진 Bald Eagle을 만날 가능성은 낮지만, 만약 그렇다면 알려주세요! 이 두 이상치를 제외한 새로운 데이터프레임을 만들어 봅시다:
...
```  
새로운 데이터프레임 `birds_filtered`를 만들고 산점도를 플롯했습니다. 이상치를 필터링함으로써 데이터가 더 일관되고 이해하기 쉬워졌습니다.
...
이제 날개 길이에 관한 데이터셋이 더 깨끗해졌으니, 이 새들에 대해 더 많은 것을 알아봅시다.

선형 플롯과 산점도는 데이터 값과 분포에 대한 정보를 표시할 수 있지만, 이 데이터셋에 내재된 값에 대해 생각해 봅시다. 다음과 같은 수량에 대한 질문을 시각화하여 답을 찾을 수 있습니다:

> 새의 카테고리는 몇 가지이며, 각각의 수는 얼마인가요?  
> 멸종, 위험, 희귀, 일반적인 새는 몇 마리인가요?  
> Linnaeus의 용어로 다양한 속(genus)과 목(order)은 몇 가지인가요?  
## 막대 차트 탐구하기
...
```  
다음 코드 스니펫에서는 데이터를 조작하고 그룹화하여 누적 막대 차트를 플롯하기 위해 [dplyr](https://www.rdocumentation.org/packages/dplyr/versions/0.7.8)과 [lubridate](https://www.rdocumentation.org/packages/lubridate/versions/1.8.0) 패키지를 설치합니다. 먼저 새의 `Category`로 데이터를 그룹화한 후 `MinLength`, `MaxLength`, `MinBodyMass`, `MaxBodyMass`, `MinWingspan`, `MaxWingspan` 열을 요약합니다. 그런 다음 `ggplot2` 패키지를 사용하여 막대 차트를 플롯하고 각 카테고리의 색상과 레이블을 지정합니다.

![Stacked bar chart](../../../../../3-Data-Visualization/R/09-visualization-quantities/images/stacked-bar-chart.png)

하지만 이 막대 차트는 너무 많은 비그룹화된 데이터로 인해 읽기 어렵습니다. 플롯하려는 데이터를 선택해야 합니다. 새의 카테고리를 기준으로 길이를 살펴봅시다.

데이터를 새의 카테고리만 포함하도록 필터링하세요.

카테고리가 많으므로 이 차트를 수직으로 표시하고 모든 데이터를 포함할 수 있도록 높이를 조정하세요:
...
```  
먼저 `Category` 열의 고유 값을 계산한 후 이를 새 데이터프레임 `birds_count`로 정렬합니다. 이 정렬된 데이터를 동일한 수준으로 팩터링하여 정렬된 방식으로 플롯합니다. 그런 다음 `ggplot2`를 사용하여 데이터를 막대 차트로 플롯합니다. `coord_flip()`은 수평 막대를 플롯합니다.

![category-length](../../../../../3-Data-Visualization/R/09-visualization-quantities/images/category-length.png)

이 막대 차트는 각 카테고리의 새 수를 잘 보여줍니다. 한눈에 미네소타 지역에서 가장 많은 새가 Ducks/Geese/Waterfowl 카테고리에 속한다는 것을 알 수 있습니다. 미네소타는 '10,000개의 호수의 땅'이므로 이는 놀랍지 않습니다!

✅ 이 데이터셋에서 다른 카운트를 시도해 보세요. 놀라운 점이 있나요?

## 데이터 비교

새로운 축을 생성하여 그룹화된 데이터의 다양한 비교를 시도할 수 있습니다. 새의 카테고리를 기준으로 MaxLength를 비교해 보세요:
...
여기서 놀라운 점은 없습니다: 벌새는 펠리컨이나 기러기에 비해 MaxLength가 가장 작습니다. 데이터가 논리적으로 맞아떨어지는 것은 좋은 일입니다!

막대 차트를 더 흥미롭게 시각화하려면 데이터를 중첩하여 표시할 수 있습니다. 특정 새 카테고리에서 최소 및 최대 길이를 중첩하여 표시해 봅시다:
