...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-24T13:12:58+00:00",
...
|:---:|
| 클라우드에서의 데이터 과학: Azure ML SDK - _Sketchnote by [@nitya](https://twitter.com/nitya)_ |
...
- [클라우드에서의 데이터 과학: "Azure ML SDK" 방식](../../../../5-Data-Science-In-Cloud/19-Azure)
  ...
  - [1. 소개](../../../../5-Data-Science-In-Cloud/19-Azure)
    ...
    - [1.2 심부전 예측 프로젝트 및 데이터셋 소개](../../../../5-Data-Science-In-Cloud/19-Azure)
...
[이전 강의](../18-Low-Code/README.md)에서는 Low code/No code 방식으로 모델을 학습, 배포 및 소비하는 방법을 살펴보았습니다. 심부전 데이터셋을 사용하여 심부전 예측 모델을 생성했습니다. 이번 강의에서는 동일한 작업을 Azure Machine Learning SDK를 사용하여 수행할 것입니다.

![프로젝트 스키마](../../../../5-Data-Science-In-Cloud/19-Azure/images/project-schema.PNG)

### 1.2 심부전 예측 프로젝트 및 데이터셋 소개

심부전 예측 프로젝트 및 데이터셋 소개는 [여기](../18-Low-Code/README.md)를 참조하세요.
...
![컴퓨트 인스턴스](../../../../5-Data-Science-In-Cloud/19-Azure/images/compute-instance-1.PNG)
...
노트북은 데이터 과학 과정에서 매우 중요한 역할을 합니다. 탐색적 데이터 분석(EDA)을 수행하거나, 컴퓨트 클러스터를 호출하여 모델을 학습하거나, 추론 클러스터를 호출하여 엔드포인트를 배포할 수 있습니다.
...
![노트북](../../../../5-Data-Science-In-Cloud/19-Azure/images/notebook-1.PNG)
3. 새로운 브라우저 탭이 열리며 Jupyter Notebook 인스턴스가 표시됩니다. "New" 버튼을 클릭하여 노트북을 생성하세요.

![노트북](../../../../5-Data-Science-In-Cloud/19-Azure/images/notebook-2.PNG)
...
이 코드는 워크스페이스를 나타내는 `Workspace` 객체를 반환합니다. 그런 다음 다음 코드를 사용하여 `실험`을 생성해야 합니다:
...
AutoML 설정을 위해 [AutoMLConfig 클래스](https://docs.microsoft.com/python/api/azureml-train-automl-client/azureml.train.automl.automlconfig(class)?WT.mc_id=academic-77958-bethanycheum&ocid=AID3041109)를 사용하세요.
...
- `task`: 실행할 작업 유형. 'classification', 'regression', 'forecasting' 값 중 하나를 선택.
...
- `featurization`: 자동화된 특징화 단계 수행 여부 또는 사용자 정의 특징화 사용 여부.
...
```
구성이 완료되었으면 다음 코드를 사용하여 모델을 학습시킬 수 있습니다. 클러스터 크기에 따라 이 단계는 최대 한 시간이 걸릴 수 있습니다.
...
```
최적 모델에 사용된 매개변수를 보려면 `fitted_model`을 출력하고 [get_properties()](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run(class)?view=azure-ml-py#azureml_core_Run_get_properties?WT.mc_id=academic-77958-bethanycheum&ocid=AID3041109) 메서드를 사용하여 최적 모델의 속성을 확인하세요.
...
축하합니다! Azure ML SDK를 사용하여 Azure ML에서 모델을 배포하고 학습시킨 후 이를 소비하는 과정을 완료하셨습니다!
...
이번 강의에서는 Azure ML SDK를 사용하여 클라우드에서 심부전 위험을 예측하는 모델을 학습, 배포, 소비하는 방법을 배웠습니다. Azure ML SDK에 대한 추가 정보를 원한다면 이 [문서](https://docs.microsoft.com/python/api/overview/azure/ml/?view=azure-ml-py?WT.mc_id=academic-77958-bethanycheum&ocid=AID3041109)를 확인해 보세요. Azure ML SDK를 사용하여 자신만의 모델을 만들어 보세요.
