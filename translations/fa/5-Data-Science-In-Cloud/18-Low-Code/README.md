...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-24T00:23:58+00:00",
...
|:---:|
| علم داده در فضای ابری: کد کم - _طرح توسط [@nitya](https://twitter.com/nitya)_ |
...
Azure ML ابزارهای مورد نیاز توسعه‌دهندگان و دانشمندان داده برای جریان‌های کاری یادگیری ماشین را فراهم می‌کند. این ابزارها شامل موارد زیر هستند:

- **Azure Machine Learning Studio**: یک پورتال وب در Azure Machine Learning برای گزینه‌های کد کم و بدون کد برای آموزش مدل، استقرار، خودکارسازی، ردیابی و مدیریت دارایی‌ها. این استودیو با Azure Machine Learning SDK یکپارچه شده است تا تجربه‌ای یکپارچه ارائه دهد.
- **Jupyter Notebooks**: برای نمونه‌سازی سریع و آزمایش مدل‌های یادگیری ماشین.
...
بیماری‌های قلبی-عروقی (CVDs) علت شماره 1 مرگ و میر در سراسر جهان هستند و 31% از کل مرگ و میرهای جهانی را به خود اختصاص می‌دهند. عوامل خطر محیطی و رفتاری مانند مصرف دخانیات، رژیم غذایی ناسالم و چاقی، فعالیت بدنی کم و مصرف مضر الکل می‌توانند به عنوان ویژگی‌هایی برای مدل‌های تخمینی استفاده شوند. توانایی تخمین احتمال توسعه CVD می‌تواند برای جلوگیری از حملات در افراد پرخطر بسیار مفید باشد.
...
| 2  | anaemia                   | بولی           | کاهش سلول‌های قرمز خون یا هموگلوبین                 | 0 یا 1            |
...
2. گزینه **＋ایجاد یک منبع** را انتخاب کنید.
   ...
   ![فضای کاری-1](../../../../5-Data-Science-In-Cloud/18-Low-Code/images/workspace-1.PNG)

   Machine Learning را جستجو کنید و کاشی Machine Learning را انتخاب کنید.

   ![فضای کاری-2](../../../../5-Data-Science-In-Cloud/18-Low-Code/images/workspace-2.PNG)

   دکمه ایجاد را کلیک کنید.

   ![فضای کاری-3](../../../../5-Data-Science-In-Cloud/18-Low-Code/images/workspace-3.PNG)
   ...
   - Key vault: توجه به Key vault جدیدی که برای فضای کاری شما ایجاد خواهد شد
   - Application insights: توجه به منبع جدید Application insights که برای فضای کاری شما ایجاد خواهد شد
   - Container registry: هیچ (یکی به طور خودکار اولین بار که مدل را به یک کانتینر مستقر می‌کنید ایجاد خواهد شد)

    ![فضای کاری-4](../../../../5-Data-Science-In-Cloud/18-Low-Code/images/workspace-4.PNG)
...
4. در صفحه نمای کلی فضای کاری خود، Azure Machine Learning studio را راه‌اندازی کنید (یا یک تب جدید مرورگر باز کنید و به https://ml.azure.com بروید)، و با استفاده از حساب Microsoft خود وارد Azure Machine Learning studio شوید. اگر درخواست شد، دایرکتوری و اشتراک Azure خود و فضای کاری Azure Machine Learning خود را انتخاب کنید.
   
![فضای کاری-5](../../../../5-Data-Science-In-Cloud/18-Low-Code/images/workspace-5.PNG)
...
![فضای کاری-6](../../../../5-Data-Science-In-Cloud/18-Low-Code/images/workspace-6.PNG)
...
- **Compute Instances**: ایستگاه‌های کاری توسعه که دانشمندان داده می‌توانند برای کار با داده‌ها و مدل‌ها استفاده کنند. این شامل ایجاد یک ماشین مجازی (VM) و راه‌اندازی یک نمونه دفترچه یادداشت است. سپس می‌توانید یک مدل را با فراخوانی یک خوشه محاسباتی از دفترچه یادداشت آموزش دهید.
- **Compute Clusters**: خوشه‌های مقیاس‌پذیر ماشین‌های مجازی برای پردازش کد آزمایش به صورت درخواستی. شما به آن نیاز خواهید داشت زمانی که یک مدل را آموزش می‌دهید. خوشه‌های محاسباتی همچنین می‌توانند منابع تخصصی GPU یا CPU را به کار گیرند.
- **Inference Clusters**: اهداف استقرار برای خدمات پیش‌بینی که از مدل‌های آموزش‌دیده شما استفاده می‌کنند.
...
تفاوت اصلی بین معماری CPU و GPU این است که CPU برای انجام سریع طیف گسترده‌ای از وظایف طراحی شده است (که با سرعت کلاک CPU اندازه‌گیری می‌شود)، اما در هم‌زمانی وظایف محدود است. در مقابل، GPU برای محاسبات موازی طراحی شده و بنابراین برای وظایف یادگیری عمیق بسیار بهتر است.
...
کلاسترهای بزرگ‌تر گران‌تر هستند اما پاسخگویی بهتری ارائه می‌دهند. بنابراین، اگر زمان دارید اما بودجه کافی ندارید، باید با یک کلاستر کوچک شروع کنید. برعکس، اگر بودجه دارید اما زمان کافی ندارید، باید با یک کلاستر بزرگ‌تر شروع کنید.
...
در [محیط کاری Azure ML](https://ml.azure.com/) که قبلاً ایجاد کردیم، به بخش Compute بروید و می‌توانید منابع محاسباتی مختلفی که قبلاً بحث کردیم (مانند نمونه‌های محاسباتی، کلاسترهای محاسباتی، کلاسترهای استنتاج و منابع محاسباتی متصل) را مشاهده کنید. برای این پروژه، ما به یک کلاستر محاسباتی برای آموزش مدل نیاز داریم. در Studio، روی منوی "Compute" کلیک کنید، سپس به تب "Compute cluster" بروید و روی دکمه "+ New" کلیک کنید تا یک کلاستر محاسباتی ایجاد کنید.
...
4. گزینه‌های خود را انتخاب کنید: حداقل/حداکثر تعداد نودها، ثانیه‌های بیکار قبل از کاهش مقیاس، دسترسی SSH. توجه داشته باشید که اگر حداقل تعداد نودها 0 باشد، در زمان بیکار بودن کلاستر پول صرفه‌جویی می‌کنید. همچنین توجه داشته باشید که هرچه تعداد نودهای حداکثری بیشتر باشد، آموزش کوتاه‌تر خواهد بود. تعداد حداکثری نودهای پیشنهادی 3 است.  
...
عالی! حالا که مجموعه داده در دسترس است و کلاستر محاسباتی ایجاد شده، می‌توانیم آموزش مدل را شروع کنیم!
...
18. با اجرای اسکریپت، باید خروجی زیر را مشاهده کنید:  
    ...
    b'"{\\"result\\": [true]}"'
    ```  
این به این معناست که پیش‌بینی نارسایی قلبی برای داده‌های داده شده درست است. این منطقی است زیرا اگر به داده‌های به‌طور خودکار تولید شده در اسکریپت نگاه کنید، همه چیز به‌طور پیش‌فرض 0 و false است. می‌توانید داده‌ها را با نمونه ورودی زیر تغییر دهید:
...
```  
اسکریپت باید این خروجی را برگرداند:  
    ```python
    b'"{\\"result\\": [true, false]}"'
    ```  
