...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-04T13:40:14+00:00",
...
Embora bases de dados ofereçam formas muito eficientes de armazenar e consultar dados usando linguagens de consulta, a maneira mais flexível de processar dados é escrever o seu próprio programa para manipulá-los. Em muitos casos, realizar uma consulta em uma base de dados seria mais eficaz. No entanto, em alguns casos onde é necessário um processamento de dados mais complexo, isso não pode ser feito facilmente usando SQL.  
...
Agora suponha que, a cada semana, organizamos uma festa para amigos e levamos 10 pacotes adicionais de gelados para a festa. Podemos criar outra série, indexada por semana, para demonstrar isso:  
```python
...
O primeiro problema em que nos vamos concentrar é a modelação da propagação epidémica da COVID-19. Para isso, usaremos os dados sobre o número de indivíduos infetados em diferentes países, fornecidos pelo [Center for Systems Science and Engineering](https://systems.jhu.edu/) (CSSE) da [Johns Hopkins University](https://jhu.edu/). O conjunto de dados está disponível neste [repositório GitHub](https://github.com/CSSEGISandData/COVID-19).

Como queremos demonstrar como lidar com dados, convidamo-lo a abrir [`notebook-covidspread.ipynb`](notebook-covidspread.ipynb) e lê-lo de cima para baixo. Também pode executar as células e realizar alguns desafios que deixámos para si no final.
...
* Extrair palavras-chave de texto e verificar com que frequência essas palavras-chave aparecem
...
Abra [`notebook-papers.ipynb`](notebook-papers.ipynb) e leia-o de cima para baixo. Também pode executar as células e realizar alguns desafios que deixámos para si no final.
...
Quer tenha dados estruturados ou não estruturados, usando Python pode realizar todos os passos relacionados com o processamento e compreensão de dados. É provavelmente a forma mais flexível de processar dados, e é por isso que a maioria dos cientistas de dados usa Python como a sua ferramenta principal. Aprender Python em profundidade é provavelmente uma boa ideia se estiver sério sobre a sua jornada na ciência de dados!
