...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-04T22:38:34+00:00",
...
Для простоти ми будемо працювати в блокноті Jupyter. Це означає, що у вас вже є робочий простір та обчислювальний екземпляр. Якщо у вас вже є робочий простір, ви можете перейти безпосередньо до розділу 2.3 Створення блокнота.
...
Давайте створимо обчислювальний екземпляр для забезпечення блокнота Jupyter. 
1. Натисніть кнопку + New. 
2. Дайте ім'я вашому обчислювальному екземпляру.
3. Виберіть ваші параметри: CPU або GPU, розмір віртуальної машини та кількість ядер.
...
Зверніться до [попереднього уроку](../18-Low-Code/README.md) у розділі **2.3 Завантаження набору даних**, якщо ви ще не завантажили набір даних.

### 2.4 Створення блокнотів

> **_Примітка:_** Для наступного кроку ви можете або створити новий блокнот з нуля, або завантажити [блокнот, який ми створили](../../../../5-Data-Science-In-Cloud/19-Azure/notebook.ipynb) у вашому Azure ML Studio. Щоб завантажити його, просто натисніть на меню "Notebook" і завантажте блокнот.

Блокноти є дуже важливою частиною процесу науки про дані. Вони можуть бути використані для проведення дослідження даних (EDA), виклику обчислювального кластеру для навчання моделі, виклику кластеру для розгортання кінцевої точки.

Щоб створити блокнот, нам потрібен обчислювальний вузол, який забезпечує екземпляр блокнота Jupyter. Поверніться до [робочого простору Azure ML](https://ml.azure.com/) і натисніть на Обчислювальні екземпляри. У списку обчислювальних екземплярів ви повинні побачити [обчислювальний екземпляр, який ми створили раніше](../../../../5-Data-Science-In-Cloud/19-Azure). 
...
Ви можете отримати набір даних з робочого простору за назвою набору даних наступним чином:
...
- `training_data`: Навчальні дані, які будуть використовуватися в експерименті. Вони повинні містити як навчальні ознаки, так і колонку міток (опціонально колонку ваг зразків).
- `label_column_name`: Назва колонки міток.
...
- `featurization`: Індикатор того, чи слід автоматично виконувати етап феатуризації, чи використовувати налаштовану феатуризацію.
...
```
Тепер, коли конфігурація налаштована, ви можете навчати модель за допомогою наступного коду. Цей крок може зайняти до години залежно від розміру кластеру.
...
```
А потім ви можете надіслати цей введений зразок до вашої моделі для прогнозування:
```python
...
**ПІДКАЗКА:** Перейдіть до [документації SDK](https://docs.microsoft.com/python/api/overview/azure/ml/?view=azure-ml-py?WT.mc_id=academic-77958-bethanycheum&ocid=AID3041109) і введіть ключові слова в пошуковий рядок, наприклад "Pipeline". У результатах пошуку ви повинні знайти клас `azureml.pipeline.core.Pipeline`.
