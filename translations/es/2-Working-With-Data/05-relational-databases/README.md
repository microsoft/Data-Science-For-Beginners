...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-23T23:22:24+00:00",
...
## [Cuestionario previo a la lección](https://purple-hill-04aebfb03.1.azurestaticapps.net/quiz/8)

## Todo comienza con tablas

Una base de datos relacional tiene como núcleo las tablas. Al igual que en una hoja de cálculo, una tabla es una colección de columnas y filas. La fila contiene los datos o información con la que deseamos trabajar, como el nombre de una ciudad o la cantidad de lluvia. Las columnas describen los datos que almacenan.
...
## Las limitaciones de una sola tabla

Es probable que la tabla anterior te resulte relativamente familiar. Comencemos a agregar algunos datos adicionales a nuestra incipiente base de datos: la lluvia anual (en milímetros). Nos centraremos en los años 2018, 2019 y 2020. Si lo agregáramos para Tokio, podría verse algo así:
...
Aunque esto evita la duplicación de filas, agrega un par de desafíos. Tendríamos que modificar la estructura de nuestra tabla cada vez que haya un nuevo año. Además, a medida que nuestros datos crecen, tener los años como columnas hará que sea más complicado recuperar y calcular valores.
...
Volvamos a nuestros datos y determinemos cómo queremos dividirlos. Sabemos que queremos almacenar el nombre y el país de nuestras ciudades, por lo que esto probablemente funcione mejor en una tabla.
...
Pero antes de crear la siguiente tabla, necesitamos averiguar cómo referenciar cada ciudad. Necesitamos algún tipo de identificador, ID o (en términos técnicos de bases de datos) una clave primaria. Una clave primaria es un valor utilizado para identificar una fila específica en una tabla. Aunque esto podría basarse en un valor en sí mismo (podríamos usar el nombre de la ciudad, por ejemplo), casi siempre debería ser un número u otro identificador. No queremos que el ID cambie nunca, ya que rompería la relación. En la mayoría de los casos, la clave primaria o ID será un número autogenerado.
...
Observa la columna **city_id** dentro de la tabla **lluvia** recién creada. Esta columna contiene valores que hacen referencia a los IDs en la tabla **ciudades**. En términos técnicos de datos relacionales, esto se llama una **clave foránea**; es una clave primaria de otra tabla. Puedes pensar en ella como una referencia o un puntero. **city_id** 1 hace referencia a Tokio.
...
Hasta ahora hemos recuperado datos de una sola tabla. Ahora queremos unir los datos de **ciudades** y **lluvia**. Esto se hace *uniéndolos*. Efectivamente, crearás un vínculo entre las dos tablas y emparejarás los valores de una columna de cada tabla.

En nuestro ejemplo, emparejaremos la columna **city_id** en **lluvia** con la columna **city_id** en **ciudades**. Esto emparejará el valor de lluvia con su respectiva ciudad. El tipo de unión que realizaremos se llama una unión *interna*, lo que significa que si alguna fila no coincide con nada de la otra tabla, no se mostrará. En nuestro caso, cada ciudad tiene datos de lluvia, por lo que todo se mostrará.

Vamos a recuperar la lluvia de 2019 para todas nuestras ciudades.

Lo haremos en pasos. El primer paso es unir los datos indicando las columnas para el vínculo - **city_id** como se destacó antes.
...
Hemos destacado las dos columnas que queremos y el hecho de que queremos unir las tablas mediante **city_id**. Ahora podemos agregar la declaración `WHERE` para filtrar solo el año 2019.
...
## [Cuestionario posterior a la lección](https://purple-hill-04aebfb03.1.azurestaticapps.net/quiz/9)
