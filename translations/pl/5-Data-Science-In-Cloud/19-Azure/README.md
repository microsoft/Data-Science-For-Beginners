...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-24T00:35:35+00:00",
...
- [Data Science w Chmurze: Podejście "Azure ML SDK"](../../../../5-Data-Science-In-Cloud/19-Azure)
  ...
  - [1. Wprowadzenie](../../../../5-Data-Science-In-Cloud/19-Azure)
    ...
    - [1.2 Projekt predykcji niewydolności serca i wprowadzenie do zbioru danych](../../../../5-Data-Science-In-Cloud/19-Azure)
...
- Zarządzanie zasobami w chmurze w celu monitorowania, logowania i organizowania eksperymentów uczenia maszynowego.
- Trenowanie modeli lokalnie lub z wykorzystaniem zasobów w chmurze, w tym przyspieszonego trenowania modeli na GPU.
...
- Wdrażanie usług internetowych, aby przekształcić wytrenowane modele w usługi RESTful, które mogą być używane w dowolnej aplikacji.
...
W [poprzedniej lekcji](../18-Low-Code/README.md) zobaczyliśmy, jak trenować, wdrażać i korzystać z modelu w sposób Low code/No code. Użyliśmy zbioru danych dotyczącego niewydolności serca, aby wygenerować model predykcji niewydolności serca. W tej lekcji zrobimy dokładnie to samo, ale używając Azure Machine Learning SDK.

![schemat-projektu](../../../../5-Data-Science-In-Cloud/19-Azure/images/project-schema.PNG)

### 1.2 Projekt predykcji niewydolności serca i wprowadzenie do zbioru danych

Sprawdź [tutaj](../18-Low-Code/README.md) wprowadzenie do projektu predykcji niewydolności serca i zbioru danych.

## 2. Trenowanie modelu za pomocą Azure ML SDK
### 2.1 Tworzenie przestrzeni roboczej Azure ML

Dla uproszczenia będziemy pracować w notatniku Jupyter. Oznacza to, że musisz już mieć przestrzeń roboczą i instancję obliczeniową. Jeśli już masz przestrzeń roboczą, możesz przejść bezpośrednio do sekcji 2.3 Tworzenie notatnika.
...
Utwórzmy instancję obliczeniową, aby uruchomić notatnik Jupyter. 
...
Gratulacje, właśnie utworzyłeś instancję obliczeniową! Użyjemy tej instancji do utworzenia notatnika w sekcji [Tworzenie Notatników](../../../../5-Data-Science-In-Cloud/19-Azure).

### 2.3 Ładowanie zbioru danych
Jeśli jeszcze nie załadowałeś zbioru danych, odnieś się do sekcji **2.3 Ładowanie zbioru danych** w [poprzedniej lekcji](../18-Low-Code/README.md).

### 2.4 Tworzenie Notatników

> **_UWAGA:_** W następnym kroku możesz utworzyć nowy notatnik od podstaw lub załadować [notatnik, który stworzyliśmy](../../../../5-Data-Science-In-Cloud/19-Azure/notebook.ipynb) w Azure ML Studio. Aby go załadować, po prostu kliknij menu "Notebook" i załaduj notatnik.

Notatniki są bardzo ważnym elementem procesu data science. Mogą być używane do przeprowadzania eksploracyjnej analizy danych (EDA), wywoływania klastra obliczeniowego w celu trenowania modelu, a także klastra inferencyjnego do wdrażania punktu końcowego.

Aby utworzyć notatnik, potrzebujemy węzła obliczeniowego, który obsługuje instancję notatnika Jupyter. Wróć do [przestrzeni roboczej Azure ML](https://ml.azure.com/) i kliknij Instancje obliczeniowe. Na liście instancji obliczeniowych powinieneś zobaczyć [instancję obliczeniową, którą utworzyliśmy wcześniej](../../../../5-Data-Science-In-Cloud/19-Azure). 
...
3. Powinno to otworzyć nową kartę przeglądarki z instancją notatnika Jupyter. Kliknij przycisk "Nowy", aby utworzyć notatnik.
...
Przede wszystkim, jeśli masz jakiekolwiek wątpliwości, odnieś się do [dokumentacji Azure ML SDK](https://docs.microsoft.com/python/api/overview/azure/ml?WT.mc_id=academic-77958-bethanycheum&ocid=AID3041109). Zawiera ona wszystkie niezbędne informacje, aby zrozumieć moduły, które omówimy w tej lekcji.
...
To zwraca obiekt typu `Workspace`, który reprezentuje przestrzeń roboczą. Następnie musisz utworzyć `experiment` za pomocą następującego kodu:
...
```
Aby uzyskać lub utworzyć eksperyment w przestrzeni roboczej, należy zażądać eksperymentu, podając jego nazwę. Nazwa eksperymentu musi mieć od 3 do 36 znaków, zaczynać się od litery lub cyfry i może zawierać tylko litery, cyfry, podkreślenia i myślniki. Jeśli eksperyment nie zostanie znaleziony w przestrzeni roboczej, zostanie utworzony nowy eksperyment.

Teraz musisz utworzyć klaster obliczeniowy do trenowania za pomocą następującego kodu. Zwróć uwagę, że ten krok może zająć kilka minut. 
...
- `max_concurrent_iterations`: Maksymalna liczba równoczesnych iteracji treningowych dozwolonych dla eksperymentu.
- `primary_metric`: Główny wskaźnik używany do określenia statusu eksperymentu.
- `compute_target`: Cel obliczeniowy Azure Machine Learning, na którym uruchamiany jest eksperyment AutoML.
...
- `enable_early_stopping`: Czy włączyć wczesne zatrzymanie, jeśli wynik nie poprawia się w krótkim okresie.
- `featurization`: Wskaźnik, czy etap featuryzacji powinien być wykonywany automatycznie, czy też należy użyć dostosowanej featuryzacji.
- `debug_log`: Plik dziennika, do którego zapisywane są informacje debugowania.
...
```
Teraz, gdy masz skonfigurowane ustawienia, możesz trenować model za pomocą następującego kodu. Ten krok może zająć do godziny, w zależności od rozmiaru klastra.
...
```
Następnie możesz wysłać te dane wejściowe do swojego modelu, aby uzyskać przewidywania:
```python
response = aci_service.run(input_data=test_sample)
response
```
Powinno zwrócić `'{"result": [false]}'`. Oznacza to, że dane pacjenta, które przesłaliśmy do punktu końcowego, wygenerowały predykcję `false`, co oznacza, że ta osoba prawdopodobnie nie dozna zawału serca.
